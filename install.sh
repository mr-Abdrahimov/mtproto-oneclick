...
print_final_info_telemt() {
  ...
  log "Префикс ee в secret= — формат Telegram для TLS-режима; внутри тот же сырой ключ, что в /etc/telemt/user-secret, плюс служебные поля (домен SNI)."
  log ""
...
prompt_telemt_proxy_tag() {
  ...
  log "${SECRET}"
  log ""
  log "В ссылке tg:// в параметре secret= длинная строка с префиксом ee — это не «новый пароль»: тот же user1-ключ в кодировке Fake-TLS плюс домен маскировки. Боту нужен только сырой 32 hex ниже."
  log "Ссылку, которую пришлёт бот после регистрации, в Telemt не используйте — подключайтесь ссылкой из API/логов сервера."
  ...
  if systemctl is-active --quiet telemt.service; then
    ...
    log ""
    sleep 2
    telemt_log_api_links "Ссылка для клиента после ad_tag (сырой user1-секрет тот же; в tg:// он в виде ee… + домен — это нормально):"
    log "Секрет для @MTProxybot не меняется от ввода ad_tag — тот же 32 hex в /etc/telemt/user-secret."
    log ""
    log "Если Telegram пишет «прокси недоступен»:"
    log "  • Добавьте в клиент ссылку из блока выше; при смене IP/порта или после новой генерации user1 ссылка меняется, но ee… — всё ещё кодировка того же ключа, пока вы не сменили user-secret."
